...
# HTTP 400 + 响应体含 code=10001 = 今日已签到（接口幂等拒绝，视为成功）
# 单独前置判断，避免 ConvertFrom-Json 解析 400 响应偶发异常导致误报失败
if ($HttpCode2 -eq "400" -and $Result -match "10001") {
    Write-Log "今日已签到，无需重复领取（接口返回已签到）"; exit 0
}
